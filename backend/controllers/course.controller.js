...
export const getLecturesByCourseId = asyncHandler(async (req, res, next) => {
        try {
            ...
            return next(new AppError(`Course lecture not loded : ${error}`, 404));
...
export const deleteCourseById = asyncHandler(async (req, res, next) => {
  try {
    ...
    // Remove course
   const deletedItem=await course.deleteOne({ _id: id });
    ...
    res.status(200).json({
     ...
     return next(new AppError(`Failed delete course,Try again.. ${error}`,400))
  }
});
...
export const editLectureByCourseIdAndLectureId=asyncHandler(async(req,res,next)=>{
     ...
     console.log("Couress id : ",req.params)
      ...
      res.status(200).json({
        success: true,
        message: 'Course lecture edited successfully',
